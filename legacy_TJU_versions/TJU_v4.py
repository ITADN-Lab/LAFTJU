...
class TJU_v4(Optimizer):
    r"""
    Corrected TJU_AdamW optimizer: maintains the precision of the original TJU_v3 while implementing proper decoupled weight decay

    Key improvements:
    1. Fix weight decay application: scale using current learning rate (current_lr * weight_decay)
    2. Preserve the approximate Hessian handling logic from the original TJU_v3
    ...
    def __init__(
        ...
        if weight_decay_type not in ['L2', 'stable', 'AdamW']:  # Fix option list
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # ====== Learning rate schedule (maintain original TJU_v3 logic) ======
                ...
                # (3) Bias correction (critical! maintain original TJU_v3 implementation)
                bias_corr1 = 1 - beta1 ** step
                bias_corr2 = 1 - beta2 ** step
                step_size = current_lr / bias_corr1  # Merge learning rate with first-order bias correction

                # (4) Approximate Hessian handling (maintain original TJU_v3 clamp logic)
                delta_grad = grad - (exp_avg / bias_corr1)  # Corrected gradient change
                ...
                # (5) Combine second-order momentum (maintain original v3 hybrid logic)
                denom = (exp_avg_sq.sqrt() / math.sqrt(bias_corr2)).add_(
                    group['hessian_scale'] * denom_hessian,
                    alpha=1.0
                ).add_(group['eps'])

                # (6) Compute update direction (key change! restore original v3 stability)
                update = exp_avg / denom

                # (7) Handle stable weight decay (maintain original v3 logic)
                ...
                # Apply decoupled decay during parameter update (keep independent of current learning rate)
                if group['weight_decay_type'] == 'AdamW' and group['weight_decay'] != 0:
                    p.data.mul_(1 - group['weight_decay'] * current_lr)  # Key fix: decouple from learning rate!
    ...
    def _compute_lr(self, group, step):
        """Learning rate schedule (precisely maintains original TJU_v3 implementation)"""
        ...
        return group['base_lr'] * 0.01  # Maintain original v3 post-training learning rate