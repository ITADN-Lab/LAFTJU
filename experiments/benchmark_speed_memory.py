...
BATCH_SIZE = 128
N_WARMUP = 10   # warmup batches
N_MEASURE = 600  # 600 steps to capture NS with interval=500
...
def make_optimizer(name, model):
    if name == 'Adam':
        ...
        return LAKTJU_NS(model.parameters(), lr=1e-3, weight_decay=2e-3,
                         betas=(0.9, 0.999), ns_interval=500, ns_steps=1,
                         ns_max_dim=256, min_ndim=2)
...
def main():
    loader = get_loader()
    optimizers = ['Adam', 'AdamW', 'Adan', 'LAFTJU_NS']
    results = {}
