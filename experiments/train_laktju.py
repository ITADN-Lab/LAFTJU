...
def parse_args():
    ...
    parser.add_argument('--optimizer', type=str, default='LAKTJU',
                        choices=['SGD', 'Adam', 'AdamW', 'ATJU', 'LAKTJU', 'LAKTJU_NS', 'LAKTJU_V9', 'LAKTJU_V10', 'LAKTJU_V11', 'LAKTJU_V12', 'Adan'])
    ...
    parser.add_argument('--ns_interval', type=int, default=500, help='NS orthogonalization interval')
    parser.add_argument('--ns_steps', type=int, default=1, help='NS iteration steps')
    parser.add_argument('--ns_max_dim', type=int, default=256, help='Max min-dim for NS (skip larger)')
    return parser.parse_args()

def load_cifar(dataset, batch_size, data_dir, workers):
    """Load CIFAR-10 or CIFAR-100 with standard augmentation."""
    # Use correct per-dataset normalization statistics
    if dataset == 'cifar10':
...
def build_optimizer(args, model, total_steps):
    ...
    if opt_name == 'SGD':
        ...
        lr = args.lr if args.lr else 0.003
        optimizer = LAKTJU_NS(params, lr=lr,
                              betas=(0.9, 0.999), eps=1e-8,
                              weight_decay=args.weight_decay,
                              ns_interval=args.ns_interval, ns_steps=args.ns_steps,
                              min_ndim=2, ns_max_dim=args.ns_max_dim,
