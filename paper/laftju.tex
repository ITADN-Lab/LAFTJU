...
% ============================================================
\begin{abstract}
We propose \method{} (\methodfull{}), a novel optimization algorithm that unifies curvature-aware trajectory optimization with adaptive gradient methods through a homotopy-based blending mechanism. \method{} employs Kronecker-factored preconditioning for efficient second-order curvature approximation, a tanh-based homotopy scheduler for smooth transition between optimization paths, and layer-wise adaptive coupling. On CIFAR-10 with ResNet-18, \method{} achieves a best test accuracy of \textbf{95.82\%}, surpassing Adam (93.99\%), AdamW (94.55\%), and Adan (94.42\%), while approaching SGD with momentum (95.99\%). On CIFAR-100, \method{} achieves \textbf{76.08\%}, competitive with SGD (77.04\%) and significantly outperforming Adan (66.30\%). Comprehensive ablation studies validate each component's contribution and reveal optimal hyperparameter configurations.
\end{abstract}
...
Deep neural network optimization remains a central challenge in machine learning. First-order methods such as SGD with momentum~\cite{sgd} and Adam~\cite{adam} dominate practice, while second-order methods offer faster convergence but at prohibitive computational cost. Recent work on AdamW~\cite{adamw}, Adan~\cite{adan}, and various hybrid approaches attempts to bridge this gap.
...
\begin{itemize}
    ...
    \item Comprehensive experimental evaluation on CIFAR-10 and CIFAR-100 demonstrating competitive or superior performance versus five baseline optimizers.
\end{itemize}
...
\textbf{First-order methods.} SGD with momentum~\cite{sgd} remains the default optimizer for many computer vision tasks. Adam~\cite{adam} and its weight-decay-corrected variant AdamW~\cite{adamw} are widely used for their adaptive learning rate properties. Adan~\cite{adan} introduces Nesterov momentum estimation into the Adam framework for improved convergence.
...
\textbf{Baselines.} We compare against five optimizers:
\begin{itemize}
    \item \textbf{SGD}: lr=0.1, momentum=0.9, weight\_decay=5$\times$10$^{-4}$
    ...
    \item \textbf{ATJU}: lr=0.001, a\_lr=0.0001, weight\_decay=5$\times$10$^{-4}$
\end{itemize}
...
\caption{Training dynamics on CIFAR-10 with ResNet-18. (a) Test accuracy over epochs. \method{} converges comparably to SGD while significantly outperforming Adam, AdamW, and Adan. (b) Training loss shows \method{}'s smooth convergence behavior.}
...
\midrule
SGD + Momentum      & 96.18 & 95.99$\pm$0.18 & 200 \\
AdamW               & 94.61 & 94.55$\pm$0.05 & 200 \\
Adan                & 94.52 & 94.42$\pm$0.13 & 200 \\
Adam                & 94.27 & 93.99$\pm$0.34 & 200 \\
ATJU                & 93.43 & 93.17$\pm$0.26 & 200 \\
\midrule
\method{} (200ep)   & 95.82 & 95.48$\pm$0.18 & 200 \\
\method{} (300ep)   & 95.66 & 95.59$\pm$0.08 & 300 \\
\method{}$\dagger$ (300ep) & \textbf{95.78} & 95.65$\pm$0.19 & 300 \\
\bottomrule
\end{tabular}
\end{table}
...
The gap to SGD (95.99\%) is only \textbf{0.17\%}, while \method{} offers the additional benefits of adaptive learning rates and curvature preconditioning that SGD lacks.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig1_cifar10_comparison.pdf}
\caption{CIFAR-10 test accuracy comparison across optimizers. \method{} significantly outperforms all adaptive methods and approaches SGD with momentum.}
...
\textbf{Optimizer} & \textbf{Best} & \textbf{Mean$\pm$Std} & \textbf{Epochs} \\
\midrule
SGD + Momentum      & 77.04 & 76.83$\pm$0.29 & 200 \\
\method{}           & 76.08 & 75.68$\pm$0.37 & 200 \\
Adam                & 74.46 & 74.19$\pm$0.24 & 200 \\
AdamW               & 71.59 & 71.32$\pm$0.25 & 200 \\
ATJU                & 70.72 & 70.40$\pm$0.28 & 200 \\
Adan                & 66.77 & 66.39$\pm$0.35 & 200 \\
\bottomrule
\end{tabular}
\end{table}

On CIFAR-100 (Table~\ref{tab:cifar100_main}), \method{} achieves \textbf{76.08\%}, which is competitive with SGD (77.04\%, gap of only 0.96\%) and significantly outperforms Adam (+1.89\%), AdamW (+4.97\%), and Adan (+9.78\%). The large improvement over Adan is particularly notable, as Adan struggles with the higher-complexity classification task.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig5_cifar100_comparison.pdf}
\caption{CIFAR-100 test accuracy comparison. \method{} ranks second only to SGD and dramatically outperforms Adan (+9.78\%).}
...
\textbf{Comparison with ATJU.} \method{} dramatically outperforms its predecessor ATJU (+2.65\% on CIFAR-10, +5.36\% on CIFAR-100), validating the effectiveness of Kronecker-factored preconditioning over diagonal Hessian approximation, and the tanh homotopy schedule over the original sigmoid-based approach.
...
\textbf{Why does \method{} outperform AdamW but not quite reach SGD?}

SGD with momentum and carefully tuned weight decay (0.001) achieves 95.99\% on CIFAR-10 with ResNet-18, which represents a strong baseline that is difficult to surpass. \method{}'s 95.82\% represents a gap of only 0.17\%, within the range of seed variance.

We hypothesize that the remaining gap is due to: (1) the implicit regularization effect of SGD's gradient noise, which naturally favors flat minima~\cite{smith2018dont}; and (2) the overhead of maintaining dual optimization paths, which slightly reduces the effective signal-to-noise ratio per step.

However, \method{} offers several advantages over SGD: (1) adaptive learning rates require less hyperparameter tuning; (2) curvature preconditioning accelerates convergence in the early training phase; (3) the homotopy mechanism provides a principled way to combine different optimization strategies.

\textbf{When should practitioners use \method{}?}

\method{} is most beneficial when: (1) the loss landscape has significant curvature heterogeneity across layers; (2) training involves complex architectures where SGD's global learning rate is suboptimal; (3) rapid early-phase convergence is important.

\textbf{Scalability considerations.}

The KF-PTC mechanism introduces modest overhead: storing two Kronecker factors per Conv2d/Linear layer and performing matrix inversions every $T_{\text{kf}}$ steps. For ResNet-18 (11.17M parameters), the additional memory is $<$5\% and the computation overhead is $<$10\% per epoch.
