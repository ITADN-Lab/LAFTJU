...
class ATJU(Optimizer):
    ...
    def __init__(
        ...
        # If init_lr is not provided, default to init_lr = tju_lr / 1000
        if init_lr is None:
            init_lr = tju_lr / 1000
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if step_count < warmup_steps:
                    # Linear ramp from init_lr to base_lr
                ...
                grad_TJU = p.grad  # gradient used by TJU
                grad_A = p.grad  # gradient used by A_optim
                ...
                if rebound_mode == 'belief':
                    rebound_thresh = grad_diff.norm(p=np.inf)  # dynamically set threshold based on max absolute value of current gradient difference (grad_diff)
                else:
                    rebound_thresh = 0.01
                    tju_eps = tju_eps / max(rebound_thresh, 1e-8)  # prevent division by zero
                ...
                prev_update_norm = prev_update.norm(p=4).add(tju_eps)  # compute L4 norm and add eps to prevent zero denominator
                prev_update.div_(prev_update_norm)  # normalize prev_update by its L4 norm
                ...
                delta_term = (grad_diff.div_(prev_update_norm)  # normalize grad_diff
                              .mul_(prev_update)  # compute dot product of normalized grad_diff with historical update prev_update
                ...
                if rebound_mode == 'belief':
                    ...
                    denom_h.add_(tju_eps / effective_alpha)  # prevent division by zero
                else:
                ...
                if wd_coef != 0 and wd_type != 'L2':  # if weight decay is non-zero and type is not L2 regularization
                    if wd_type == 'stable':
                        scaled_decay = wd_coef / max(denom_h.mean().item(), 1e-8)  # compute mean of denom_h (reflects parameter curvature).
                        # divide wd_coef by this mean to get scaled decay factor scaled_decay.
                        new_update.add_(p, alpha=scaled_decay)  # apply scaled decay factor to parameter p, updating direction new_update
                    else:
                        # decoupled
                ...
                p.add_(new_update)  # apply gradient update, new_update controls the update direction

                # Store the current update vector for use in the next iteration
                prev_update.copy_(new_update)  # save current update new_update to prev_update for use in next iteration
