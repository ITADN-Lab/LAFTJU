...
class ATJU(Optimizer):
    ...
    def __init__(
        ...
        # ATJU inherits all attributes from the parent class, corresponding to parent params and defaults
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            total_epoch = group['total_epoch'],
            epoch_now = group['epoch_now']

            # Define the current weight value s
            s = self.epoch_now/self.total_epoch
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    # Exponential moving average of first-order gradients (AdamW)
                    state['exp_avg_grad_AdamW'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                    # Exponential moving average of second-order gradients (AdamW)
                    state["exp_avg_sq_grad_AdamW"] = torch.zeros_like(p, memory_format=torch.preserve_format)
                ...
                # grad_sgd = p.grad   # Gradient value used by SGD
                grad_AdamW = p.grad  # Gradient value used by AdamW
                if grad.is_sparse:
                ...
                exp_avg_grad_AdamW = state['exp_avg_grad_AdamW']
                exp_avg_sq_grad_AdamW = state["exp_avg_sq_grad_AdamW"]
                ...
                grad_diff = grad_TJU - exp_avg_grad  # Current parameter gradient minus stored historical gradient
                ...
                prev_update_sq = prev_update.mul(prev_update)  # Compute the square of each element of the normalized prev_update
                ...
                if rebound_mode == 'belief':
                    ...
                    denom_h = approx_hessian.abs().clamp_(
                        min=rebound_thresh)  # Take absolute value of approx_hessian, clamp values below rebound_thresh to rebound_thresh

                new_update = exp_avg_grad.div(denom_h)  # Scale historical gradient momentum exp_avg_grad by inverse curvature to get final update direction
                ...
                updata_sgd = s * (grad_sgd + momentum * momentum_buffer)
                # Store accumulated momentum value
                state["momentum_buffer"] = grad_sgd + momentum * momentum_buffer
                '''

                # AdamW update section
                exp_avg_grad_AdamW = beta1 * exp_avg_grad_AdamW + (1 - beta1) * grad_AdamW
                exp_avg_sq_grad_AdamW = beta2 * exp_avg_sq_grad_AdamW + (1 - beta2) * grad_AdamW**2

                exp_avg_grad_AdamW_hat = exp_avg_grad_AdamW / (1 - beta1 ** step_count)
                exp_avg_sq_grad_AdamW_hat = exp_avg_sq_grad_AdamW / (1 - beta2 ** step_count)
                updata_AdamW = s * ((exp_avg_grad_AdamW_hat / (torch.sqrt(exp_avg_sq_grad_AdamW_hat) + eps)) + wd_coef*p)


                state["exp_avg_grad_AdamW"] = exp_avg_grad_AdamW
                state["exp_avg_sq_grad_AdamW"] = exp_avg_sq_grad_AdamW


                # Compute joint update direction
                # new_update = updata_TJU + updata_sgd
                new_update = updata_TJU + updata_AdamW
                p.add_(new_update, alpha=-current_lr)  # Perform gradient update; new_update controls update direction, alpha=-current_lr is the learning rate (step size)

                # Store the current update vector for use in the next iteration
                prev_update.copy_(new_update)  # Save the current update new_update to prev_update for the next iteration
